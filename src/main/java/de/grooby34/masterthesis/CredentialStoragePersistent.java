 ...
 *
 * <p>Manages long-term storage of FIDO2 credentials in JavaCard's persistent memory. Optimized for
 * space efficiency and fast credential lookup operations.
 *
 * <p>Storage format per credential: - 32 bytes: Credential ID - 32 bytes: RP ID hash - 32 bytes:
 * User ID hash - 32 bytes: Private key (P-256) - 64 bytes: Public key (P-256, uncompressed) - 4
 * bytes: Signature counter (big-endian) - 1 byte: Status flags Total: 197 bytes per credential
 *
...
public class CredentialStoragePersistent {
   ...
   */
  public short storeCredentialPersistent(
      byte[] credentialId,
      short credIdOffset,
      byte[] rpIdHash,
      short rpIdOffset,
    ...
    Util.arrayCopy(
        ...
        credIdOffset,
        credentialStorage,
        (short) (baseOffset + OFFSET_CREDENTIAL_ID),
        (short) 32);
    ...
    Util.arrayCopy(
        ...
        rpIdOffset,
        credentialStorage,
        (short) (baseOffset + OFFSET_RP_ID_HASH),
        (short) 32);
    ...
    Util.arrayCopy(
        ...
        userIdOffset,
        credentialStorage,
        (short) (baseOffset + OFFSET_USER_ID_HASH),
        (short) 32);
    ...
    if (privKeyLength == 32) {
      Util.arrayCopy(
          ...
          (short) 0,
          credentialStorage,
          (short) (baseOffset + OFFSET_PRIVATE_KEY),
          (short) 32);
    }
    ...
    if (pubKeyLength == 65 && publicKeyData[0] == 0x04) {
      ...
      Util.arrayCopy(
          ...
          (short) 1,
          credentialStorage,
          (short) (baseOffset + OFFSET_PUBLIC_KEY),
          (short) 64);
    }
  ...
  public void getAndIncrementSignatureCounter(short credentialIndex, byte[] out, short outOffset) {
    ...
    // Note: if carry is true after the loop, it means that all signcounter bits were already used
    // (overflow). As this is extremely unlikely it can be ignored.
  }
